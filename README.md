...
│   ├── static/                # Source CSS/JS — edit files here, not in staticfiles/
│   │   ├── css/                (style.css = layout, theme.css = colors/components)
│   │   └── js/
│   ├── templates/              # HTML templates (base.html + per-feature folders)
│   └── migrations/
├── crm_project/               # Django project settings, URLs, WSGI
├── staticfiles/                # GENERATED by `collectstatic` — gitignored, don't edit
